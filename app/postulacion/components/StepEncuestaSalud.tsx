...
export function StepEncuestaSalud({
  ...
  // Estándar UNIMAR Academic Minimalist (Simetría unificada)
  ...
  // Componente RadioItem estandarizado
  const CustomRadioItem = ({ value, id, label, colorClass = "text-[#1e3a5f]" }: any) => (
    <label 
      htmlFor={id}
      className={cn(
        "flex-1 flex items-center justify-center gap-2 px-4 h-10 rounded-xl border border-slate-200 bg-slate-50/50 has-[:checked]:bg-[#1e3a5f]/5 has-[:checked]:border-[#1e3a5f] transition-all cursor-pointer group",
        disabled && "opacity-60 cursor-not-allowed"
      )}
    >
      <RadioGroupItem value={value} id={id} className="h-4 w-4 text-[#1e3a5f]" />
      <span className={cn("text-[10px] font-bold uppercase cursor-pointer tracking-tight truncate", colorClass)}>
        {label}
      </span>
    </label>
  );
    ...
    <div className="flex flex-col gap-3.5 animate-in fade-in duration-500 pb-4">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
        <div className="flex items-center gap-2">
           <HeartPulse className="h-4 w-4 text-[#d4a843]" />
        ...
        {estaEnfermo ? (
          <div className="flex flex-col gap-3 pt-2 animate-in slide-in-from-top-2 duration-300">
            <div className="space-y-1">
              <Label className="text-[8px] font-black uppercase text-amber-700 flex items-center gap-1.5">
                <Stethoscope className="h-3 w-3" /> Diagnóstico Médico Detallado
              </Label>
              <Input 
            ...
            <div className="space-y-1">
              <Label className="text-[8px] font-black uppercase text-amber-700 flex items-center gap-1.5">
                <Activity className="h-3 w-3" /> Tratamiento e Insumos
              </Label>
              <Input 
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-2">
        <Label className="text-[9px] font-black uppercase text-[#1e3a5f] flex items-center gap-1.5">
          <MessageSquare className="h-3.5 w-3.5 text-[#d4a843]" /> Observaciones del Solicitante
        </Label>
        <Input 
